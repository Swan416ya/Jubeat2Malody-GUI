...
_atwiki_index: Dict[Tuple[float, float, float, float], Tuple[str, str]] = {}
...
def parse_atwiki_html_table(html_text: str) -> Dict[Tuple[float, float, float, float], Tuple[str, str]]:
    """解析 atwiki HTML 表格为 (bpm,bsc,adv,ext) → (title, artist)。"""
    index: Dict[Tuple[float, float, float, float], Tuple[str, str]] = {}
    ...
    for row in re.findall(r"<tr[^>]*>(.*?)</tr>", html_text, re.S):
        ...
        index[key] = (title, artist)
    return index


def parse_atwiki_sonicy_table(text: str) -> Dict[Tuple[float, float, float, float], Tuple[str, str]]:
    """解析 atwiki 管道符表格文本。"""
    if "<tr" in text:
...
def load_level_fingerprint_tsv(path: Path) -> Dict[Tuple[float, float, float, float], Tuple[str, str]]:
    index: Dict[Tuple[float, float, float, float], Tuple[str, str]] = {}
    if not path.is_file():
        return index
    for line in path.read_text(encoding="utf-8", errors="replace").splitlines():
        ...
        try:
            ...
            continue
        index[key] = (parts[4].strip(), parts[5].strip())
    return index
...
def lookup_atwiki_by_levels(info: dict) -> Optional[Tuple[str, str]]:
    ...
    if not sig:
        return None
    load_atwiki_index()
    return _atwiki_index.get(sig.key())
